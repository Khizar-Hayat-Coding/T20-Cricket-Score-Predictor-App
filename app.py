...
def index():
    ...
    if request.method == 'POST':
        ...
        if overs <= 5:
            error = "Overs must be greater than 5 to make a valid prediction."
            return render_template('score.html', teams=sorted(teams), cities=sorted(cities), error=error)
        ...
        input_df = pd.DataFrame(
            {'batting_team': [batting_team], 'bowling_team': [bowling_team], 'city': [city], 
             ...
             'wicket_left': [10 - wickets],'current_run_rate': [current_score / overs], 'crr': [crr], 'last_five': [last_five]}
        )

        result = pipe.predict(input_df)
        prediction = int(result[0])

    return render_template('score.html', teams=sorted(teams), cities=sorted(cities), prediction=prediction)
